<%@ Control Language="C#" AutoEventWireup="true" CodeFile="toolbarScript.ascx.cs" Inherits="prescriberreporting_controls_toolbarScript" %>
<%@ OutputCache Shared="true" VaryByParam="None" Duration="120" %>  
    ...
    <script type="text/javascript">
        ...
        function requestReportToolbar()
        {
            ...
            {
                data["Plan_ID1"] = $('#Plan_ID1_DATA').val();
                data["Plan_ID2"] = $('#Plan_ID2_DATA').val();

                data["Plan_ID1_Text"] = $('#Plan_ID1').val();
                data["Plan_ID2_Text"] = $('#Plan_ID2').val();

                //Make sure Account 1 Selection does not equal Account 2 Selection
                if (data["Plan_ID1_DATA"] && data["Plan_ID2_DATA"] && data["Plan_ID1_DATA"].value == data["Plan_ID2_DATA"].value)
                {
                ...
                if ($("#Plan_ID1_DATA").val() == "" || $("#Plan_ID2_DATA").val() == "")
                {
                    $alert("Please select both accounts to proceed.", "Account Selection");
                    isValid = false;
                }
            }
