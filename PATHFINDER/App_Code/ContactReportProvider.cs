...
public class ContactReportProvider
{
    //Used for Meeting Type and Meeting Activity report
    public void ProcessChart(Chart chart, List<DbDataRecord> meetingActivityFilter, int prodID, string reportType, int chartNum)
    {
        using ( PathfinderClientModel.PathfinderClientEntities clientContext = new PathfinderClientModel.PathfinderClientEntities(Pinsonault.Web.Session.ClientConnectionString) )
        {
            ...
            foreach (var y in meetingActivityFilter)
            {
                ...
                //Call addPoint and get values of fields by index
                addPoint(chart, prodID, drugNameText, y.GetValue(mtgIDOrdinal), y.GetValue(mtgNameOrdinal),
    ...
    public void ProcessChart(Chart chart, IList<DbDataRecord> meetingActivityFilter, string[] contactDate, int chartNum)
    {
        using ( PathfinderClientModel.PathfinderClientEntities clientContext = new PathfinderClientModel.PathfinderClientEntities(Pinsonault.Web.Session.ClientConnectionString) )
        {
            ...
            foreach (var y in meetingActivityFilter)
            {
                ...
                //Call addPoint and get values of fields by index
                addPoint(chart, y.GetValue(prodIDOrdinal), y.GetValue(prodNameOrdinal), y.GetValue(numOfCallsOrdinal), 
    ...
    //Used for Meeting Type and Meeting Activity report
    void addPoint(Chart chart, Int32 productId, string drugNameText, object meetingID, object meetingName, object Number_Of_Calls, object Percent_Of_Calls, int index, int noColumn, string reportType)
    {
    ...
    //Used for Products Discussed report
    void addPoint(Chart chart, object productId, object productName, object Number_Of_Calls, object Percent_Of_Calls, int index, int noColumn)
    {
        chart.Series[index].Type = SeriesChartType.Pie;
        //chart.Series[index].ShowInLegend = true;
        chart.Series[index].ShowInLegend = false;
    ...
    public void ProcessGrid(Telerik.Web.UI.RadGrid grid, IList<DbDataRecord> meetingActivityFilter)
    {
        using ( PathfinderClientModel.PathfinderClientEntities clientContext = new PathfinderClientModel.PathfinderClientEntities(Pinsonault.Web.Session.ClientConnectionString) )
        {
    ...
    public string[] GetProductsDiscussedID()
    {
