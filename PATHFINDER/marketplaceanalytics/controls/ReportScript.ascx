<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ReportScript.ascx.cs" Inherits="marketplaceanalytics_controls_ReportScript" %>
    ...
    function onGridClicked(sender, args)
    {
        ...
        if ($('.formularyhistoryreporting').length < 1)
        {
            ...
            if (data)
            {
                ...
                var vals = args.ID;
                var planID, defaultProductID, segmentID;

                planID = vals.toString().split("~")[0];
                defaultProductID = vals.toString().split("~")[1];
                segmentID = vals.toString().split("~")[2];
                ...
                //Affiliation: col["Plan_ID"] = Parent Plan, so replace it, Plan_ID should be selected from detailed grid
                if (col["Plan_ID"])
                    q = q + "&Default_Product_ID=" + defaultProductID + "&qSegment_ID=" + segmentID;
                
                else  // Trending, Comparison
                    q = q + "&Plan_ID=" + planID + "&Default_Product_ID=" + defaultProductID + "&qSegment_ID=" + segmentID;
                
                if(planID)
                {
                    col["Plan_ID"] = planID;   //plan id comes from selected value in grid
                    col["Default_Product_ID"] = defaultProductID;
     
                    clientManager.setContextValue("physQuery", col);
                    
                    var url = "marketplaceanalytics/all/PrescriberPopup.aspx?" + q;
