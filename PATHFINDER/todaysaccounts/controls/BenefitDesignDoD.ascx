<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BenefitDesignDoD.ascx.cs" Inherits="controls_BenefitDesignDoD" %>
<div ID="BDHeader1" class="areaHeader" runat="server">DoD</div>
     <telerik:RadGrid SkinID="radTable" runat="server" ID="gridDoDBenefitDesg" AllowPaging="false" 
     AllowFilteringByColumn="false" EnableEmbeddedSkins="false" Width="100%" DataSourceID="dsDoDBenefitDesign">        
  ...
  <%-- <div class="clDate" runat="server" id="formularyDate">
        <%= Pinsonault.Web.Support.GetDataUpdateDateByKey("DoD Formulary", Resources.Resource.Label_Section_Last_Updated)%>
   </div>--%>
 <asp:EntityDataSource ID="dsDoDBenefitDesign" runat="server" EntitySetName="BenefitDesignSet" ConnectionString="name=PathfinderEntities" DefaultContainerName="PathfinderEntities" 
