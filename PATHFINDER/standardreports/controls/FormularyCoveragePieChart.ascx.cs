...
public partial class standardreports_controls_FormularyCoveragePieChart : System.Web.UI.UserControl, IReportChart
{
    public void ProcessChart(IEnumerable<DbDataRecord> data, string chartTitle, string region)
    {
        ...
        legendOuter.Position.X = 85;
        legendOuter.Position.Y = 1;
        legendOuter.Position.Width = 18;
        ...
        if (data.Count() > 0)
        {
            ...
            if (NoDataFlag == false)
            {
                ...
                Other_Percents = new double[formularystatus.Count];
                ...
                foreach (DbDataRecord record in data)
                {
                    ...
                    if ((Array.IndexOf(selSectionIds, record.GetInt32(record.GetOrdinal("Section_ID")).ToString()) == -1) && (AllSectionFlag != true))
                    {
                        ...
                        {
                            sFS = string.Format("{0}_Percent", fstatus.Abbr);
                ...
                //then calculate dummypoint as 50% of the smallest lives.
                if (iCnt >= 4)
                ...
                foreach (DbDataRecord record in data)
                {
                    ...
                    if ((Array.IndexOf(selSectionIds, Convert.ToString(drillDownSectionID)) != -1) || (AllSectionFlag))
                    {
                        ...
                        foreach (FormularyStatus fstatus in formularystatus)
                        {
                            ...
                            sFS = string.Format("{0}_Percent", fstatus.Abbr);
                            outerSeries.Points.AddY((Convert.ToDouble(record.GetInt32(record.GetOrdinal("Formulary_Lives"))) * Convert.ToDouble(record[sFS])) / 100);
                            outerSeries.Points[i].Name = fstatus.Name;
                            outerSeries.Points[i].Color = ReportColors.StandardReportsPieDark.GetColor(icolor);

                            //Show percentages on tooltip.
                            outerSeries.Points[i].ToolTip = drugName + ", " + fstatus.Name + " \n " + String.Format("{0:0.#}", Convert.ToDouble(record[sFS])) + "%";

                            //show the legend and label for formulary status only for the firsttime.
                            if (index <= 1)
                            {
                                outerSeries.Points[i].LegendText = fstatus.Name;
                                outerSeries.Points[index].Label = fstatus.Name;
                            }
                            else
                            {
                ...
                if (OtherFSLives > 0)
                {
                    ...
                    foreach (FormularyStatus fstatus in formularystatus)
                    {
                        ...
                        //Calculate the percentage for each formulary status point in 100.
                        double OtherPercentin100 = (Convert.ToDouble(Other_Percents[iFS] * 100) / percentCalc);
                        ...
                        //Show percentages on tooltip.
                        outerSeries.Points[i].ToolTip = drugName + ", " + fstatus.Name + " \n " + String.Format("{0:0.#}", OtherPercentin100) + "%";
