...
<%@ OutputCache Shared="true" VaryByParam="None" Duration="100" %> 
 ...
 <script type="text/javascript">
    ...
    function OpenCCR(LinkClicked, CCRID, PPID) {        
        ...
        var url = null;
        if (CCRID) {
            url = "custom/reckitt/customercontactreports/all/AddEditCCReports.aspx?LinkClicked=" + LinkClicked + "&CRID=" + CCRID + "&PlanID=" + PPID + "&PlanName=" + planName;
        }
        else {
            if (planID)
                url = "custom/reckitt/customercontactreports/all/AddEditCCReports.aspx?LinkClicked=" + "AddCCR" + "&PlanID=" + planID + "&PlanName=" + planName;
            else
    ...
    {
        if (DOCID != "")
            $openWindow("custom/reckitt/customercontactreports/all/RemoveDocument.aspx?LinkClicked=" + LinkClicked + "&DocumentID=" + DOCID, null, null, 500, 110, LinkClicked);
    }
    ...
    function OpenDocUpload(LinkClicked) 
    {
        var planid = getSelectedPlanID();
        if (planid)
            $openWindow("custom/reckitt/customercontactreports/all/DocumentUpload.aspx?Plan_ID=" + planid, null, null, 650, 120, LinkClicked);
        else 
            $alert(planMessage, title, null, 50);
    }
