...
//        }
    ...
    function setMeetingOutcomeText(sender, args)
    {
        var hdnMeetOutcome = $("#ctl00_main_hdnMeetOutcome");
        var strPlans = "";

        //Get currently selected list of MeetingOutcomeIDs.
        if (hdnMeetOutcome.length > 0)
            strPlans = hdnMeetOutcome.val();

        //If list is not empty then update MeetingOutcome dropdown message to 'Change Selection' else '-Select Meeting Outcome-'.
        if (strPlans != "")
            sender.set_text("-Change Selection-");
        else
            sender.set_text("-Select Meeting Outcome-");
    }
</script>
