<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CCReportScripts.ascx.cs" Inherits="custom_auxilium_customercontactreports_controls_CCReportScripts" %>
 <script type="text/javascript">
    ...
    function pageInitialized() 
    {
        ...
        //handle the hide and show for delete and view link in business plan doc grid
    ...
    function getSelectedPlanID()
    {
        ...
        if(items && items.length > 0)
            planid = items[0].get_dataItem()["ID"]; 
    ...
    function getSelectedPlanName()
    {
        ...
        var items = get_PlanSectionSelectGrid().get_masterTableView().get_selectedItems();
        if (items && items.length > 0)
            planname = items[0].get_dataItem()["Name"];
    ...
    function DeleteCCR(LinkClicked, CCRID) {
        if (CCRID != "")
            $openWindow("custom/auxilium/customercontactreports/all/RemoveCCR.aspx?LinkClicked=" + LinkClicked + "&CCRID=" + CCRID, null, null, 500, 110, LinkClicked);
    }
    ...
    function OpenCCR(LinkClicked, CCRID, PPID) {        
        ...
        var url = null;
        if (CCRID) {
            url = "custom/auxilium/customercontactreports/all/AddEditCCReports.aspx?LinkClicked=" + LinkClicked + "&CRID=" + CCRID + "&PlanID=" + PPID + "&PlanName=" + planName;
        }
        else {
            if (planID)
                url = "custom/auxilium/customercontactreports/all/AddEditCCReports.aspx?LinkClicked=" + "AddCCR" + "&PlanID=" + planID + "&PlanName=" + planName;
            else
                $alert(planMessage, title, null, 50);
        }
    ...
    {
        if (DOCID != "")
            $openWindow("custom/auxilium/customercontactreports/all/RemoveDocument.aspx?LinkClicked=" + LinkClicked + "&DocumentID=" + DOCID, null, null, 500, 110, LinkClicked);
    }
    ...
    function viewDocument()
    {
        ...
        if (docID)
            window.open("usercontent/OpenBusinessDocument.ashx?&Document_ID=" + docID);
        else 
            $alert(docMessage, title, null, 50);
    }
    ...
    function OpenDocUpload(LinkClicked) 
    {
        var planid = getSelectedPlanID();
        if (planid)
            $openWindow("custom/auxilium/customercontactreports/all/DocumentUpload.aspx?Plan_ID=" + planid, null, null, 650, 120, LinkClicked);
        else 
            $alert(planMessage, title, null, 50);
    }
    ...
    function gridPlans_OnRowSelected(sender, args)
    {
        ...
        //load data
        var planid = args._dataKeyValues.ID;
