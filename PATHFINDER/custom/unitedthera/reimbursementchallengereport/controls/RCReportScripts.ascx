<%@ Control Language="C#" AutoEventWireup="true" CodeFile="RCReportScripts.ascx.cs" Inherits="custom_controls_CCReportScript" %>
<%@ OutputCache Shared="true" VaryByParam="None" Duration="100" %> 
 ...
 <script type="text/javascript">
    ...
    var title = "Reimbursement Challenge Report";
    ...
    function OpenRCR(LinkClicked, IRID, PPID) {
        ...
        if (IRID) {
            url = clientManager.get_ApplicationManager().get_UrlName() + "/all/AddEditRCReports.aspx?LinkClicked=" + LinkClicked + "&IRID=" + IRID + "&PlanID=" + PPID + "&PlanName=" + encodeURIComponent(planName)+ "&Profiled=" + profiled;
        }
        else {
            if (planID) {
                url = clientManager.get_ApplicationManager().get_UrlName() + "/all/AddEditRCReports.aspx?LinkClicked=" + "AddIR" + "&PlanID=" + planID + "&PlanName=" + encodeURIComponent(planName) + "&Profiled=" + profiled;
            }
            else
        ...
        if (url)
            $openWindow(url, null, null, 750, 425, LinkClicked);
    }
...
//        OpenCCR("EditCCR", args._dataKeyValues.Contact_Report_ID, args._dataKeyValues.Plan_ID);
    ...
    function OpenDeleteRCR() {
        ...
        if (items != null && items.length > 0) {
            IRID = items[0].get_dataItem()["RC_Report_ID"];

            DeleteRCR("RemoveWnd", IRID);
        
            
        }
        else {
           //$alert(planMessage, title, null, 50);
           $alert("Please Select a Reimbursement Issue Report");
        }
    }

    function DeleteRCR(LinkClicked, IRID) {
        if (IRID != "")
            $openWindow("custom/unitedthera/reimbursementchallengereport/all/RemoveRCR.aspx?LinkClicked=" + LinkClicked + "&IRID=" + IRID, null, null, 500, 110, LinkClicked);
    }
