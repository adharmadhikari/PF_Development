<%@ Page Title="" Language="C#" MasterPageFile="~/custom/MasterPages/SellSheetStep.master" AutoEventWireup="true" CodeFile="reviewplanselection.aspx.cs" Inherits="custom_unitedthera_sellsheets_reviewplanselection"  EnableViewState="true"  %>
<%@ MasterType VirtualPath="~/custom/MasterPages/SellSheetStep.master" %>
<%@ Register src="~/custom/unitedthera/sellsheets/controls/ReviewPlanSelectionScript.ascx" tagname="ReviewPlanSelectionScript" tagprefix="pinso" %>
    ...
    <asp:ListView runat="server" ID="ReviewPlansListView" 
        ...
        <ItemTemplate>
         ...
         <tr id="Tr2" runat="server"  class='<%#Eval("HighlightIndicator") %>' >
           ...
           <td width="7%" id="Tier1Data" class="PlanNMHighlight" runat="server" Visible='<%# (bool)Eval("Tier_Indicator") %>' >  
             <asp:Label ID="Tier1lbl" runat="server" Text='<%#Eval("FirstTierName") %>' />
           </td>
           <td width="7%" id="Cov1Data" class="PlanNMHighlight" runat="server" Visible='<%# (bool)Eval("Coverage_Indicator") %>' >  
             <asp:Label ID="Cov1lbl" runat="server" Text='<%#Eval("FirstCoverageStatus") %>'/>
           </td>
           <td width="7%" id="Copay1Data" runat="server" align="right" Visible='<%# (bool)Eval("Copay_Indicator") %>' class="PlanNMHighlight right">  
             <asp:Label ID="Copay1lbl" runat="server" Text='<%#Eval("FirstCopay") %>' />
           </td>
           <td width="7%" id="Tier2Data" class="PlanNMHighlight" runat="server" Visible='<%# (bool)Eval("Tier_Indicator") %>'>  
             <asp:Label ID="Tier2lbl" runat="server" Text='<%#Eval("SecondTierName") %>' />
           </td>
           <td width="7%" id="Cov2Data" class="PlanNMHighlight" runat="server"  Visible='<%# (bool)Eval("Coverage_Indicator") %>'>  
             <asp:Label ID="Cov2lbl" runat="server" Text='<%#Eval("SecondCoverageStatus") %>' />
           </td>
           <td width="7%" id="Copay2Data" runat="server" align="right" Visible='<%# (bool)Eval("Copay_Indicator") %>' class="PlanNMHighlight right">  
             <asp:Label ID="Copay2lbl" runat="server" Text='<%#Eval("SecondCopay") %>' />
           </td>
           <td width="7%" id="Tier3Data" class="PlanNMHighlight" runat="server" Visible='<%# (bool)Eval("Tier_Indicator") %>'>  
             <asp:Label ID="Tier3lbl" runat="server" Text='<%#Eval("ThirdTierName") %>' />
           </td>
           <td width="7%" id="Cov3Data" class="PlanNMHighlight" runat="server"  Visible='<%# (bool)Eval("Coverage_Indicator") %>'>  
             <asp:Label ID="Cov3lbl" runat="server" Text='<%#Eval("ThirdCoverageStatus") %>' />
           </td>
           <td width="7%" id="Copay3Data" runat="server" align="right" Visible='<%# (bool)Eval("Copay_Indicator") %>' class="PlanNMHighlight right">  
