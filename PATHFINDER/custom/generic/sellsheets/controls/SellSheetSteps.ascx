        ...
        <asp:Repeater runat="server" ID="repeater" DataSourceID="dsSteps">
            <ItemTemplate>        
                <li class='<%#Eval("Step_Order","step{0}") %>'>
                    <a href="javascript:void(0)" onclick="clientManager.set_Module('<%# Eval("Step_Key") %>')" class='<%# this.GetClassName(Eval("Step_Key") as String) %>' stepKey='<%# Eval("Step_Key") %>' stepOrder='<%# Eval("Step_Order") %>'>
